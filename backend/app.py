...
COOLDOWN_SECONDS = 10
MAX_ALERTS = 100
IDLE_CLEAR_SECONDS = 15  # ← ADD THIS
...
cooldowns = {}
last_alert_received = time.time()  # ← ADD THIS
...
threading.Thread(target=idle_watcher, daemon=True).start()  # ← ADD THIS BLOCK
...
def receive_alert():
    global last_alert_received  # ← ADD THIS LINE
    ...
    if alert_type not in ROUTING_TABLE:
        return jsonify({"error": f"Unknown type. Use: crowd, breach, distress"}), 400
    if zone not in ZONE_NAMES:
        return jsonify({"error": f"Unknown zone. Use: zone_1 to zone_5"}), 400
    if is_on_cooldown(zone, alert_type):
        return jsonify({"status": "cooldown"}), 200

    last_alert_received = time.time()  # ← ADD THIS LINE
